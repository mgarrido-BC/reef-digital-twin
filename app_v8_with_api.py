...
# Google Sheets URL for cloud deployment (lab data)
GOOGLE_SHEETS_LAB_URL = "https://docs.google.com/spreadsheets/d/1aqslSDmVTFmIESXDVeGLl5cFn2KzevX0WhfT2wi-OOU/export?format=csv"

# Detect if running on Streamlit Cloud vs locally
IS_CLOUD = os.environ.get("STREAMLIT_RUNTIME") is not None or not os.path.exists("C:/")
...
def load_lab_data():
    """Load lab data from Google Sheets (cloud) or Excel (local)"""
    try:
        if IS_CLOUD:
            # Fetch from Google Sheets when running on Streamlit Cloud
            df = pd.read_csv(GOOGLE_SHEETS_LAB_URL)
            df['Date'] = pd.to_datetime(df['Date'])
            df = df.sort_values('Date', ascending=True)
            return df, None
        else:
            # Local: use Excel file
...
def run_sumo_model():
    ...
    # SUMO only works locally (requires Windows + SUMO installation)
    if IS_CLOUD:
        return False, {}, "SUMO model is only available when running locally (requires Windows + SUMO license)"
    try:
        clear_progress()
...
def run_sumo_with_progress(chat_container):
    ...
    if IS_CLOUD:
        with chat_container:
            st.warning("🔬 SUMO model is only available when running locally (requires Windows + SUMO license)")
        return False, {}, "Model not available on cloud"
